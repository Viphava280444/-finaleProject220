...
import {islogin, nameAccount, accounts, mode, subject} from "./stores.js";
  ...
  function change(index) {
    ...
    if (confirm('คุณต้องการลงทะเบียนรายวิชาใช่หรือไม่')==true){
...
<div class="container">
  <div class="name">ระบบลงทะเบียนโควต้ารายวิชา</div>
 ...
 <main>
  ...
  {#each $subject as {sub, students, status, nameSub},index}
   ...
   <button on:click={()=>change(index)} class = 'click'>ลงทะเบียน</button>  
 </div>
   {/if}
   {/if}
